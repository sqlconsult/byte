...
`check_luhn` should run the Luhn Algorithm and return true or false.


`validate` should call the other 3 functions and determine if the card is valid or not.
...
Write your own tests to test any other possible cases where your code might fail.

To execute the tests enter this in the command line:

```bash
python3 -m unittest test_credit_card
```

For this to work you must be in the directory `2-credit-card-validator`.
